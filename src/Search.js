        ...
        return (
            <div className="App">
                ...
                <div style={{'border': "1px solid lightcoral", 'borderRadius': "4px", "padding": "10px", "margin": "10px"}} key={movie.id}>
                    <h2><Link to={`/movie/${movie.id}`}>{movie.original_title}</Link></h2>
                    {
                    movie.poster_path 
                    ? <img style={{"width": "100%"}} src={`https://image.tmdb.org/t/p/w500/${movie.poster_path}`} alt={`Poster of ${movie.original_title}`} />
                    : "No poster"}
                </div>
                ))}
            </div>
